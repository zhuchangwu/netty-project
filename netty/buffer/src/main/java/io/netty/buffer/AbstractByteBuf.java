...
public abstract class AbstractByteBuf extends ByteBuf {
    ...
    //todo 他保存了下面的读写指针， 进行计算，得到当前byteBuf的状况
    int readerIndex;
    int writerIndex;
    ...
    // todo 判断byte是否可以继续读, 就是writeInedx>readerIndex时
    @Override
    ...
    // todo 判断是否是可写的, 就是 capacity > writerIndex
    @Override
    ...
    public ByteBuf discardReadBytes() {
        ...
        if (readerIndex != writerIndex) {
            ...
            writerIndex -= readerIndex; // todo 将writeIndex - readerIndex 得到的就是中间的
            adjustMarkers(readerIndex);
            readerIndex = 0;  // todo 重置readerindex
        } else {
            adjustMarkers(readerIndex);
    ...
    final void ensureWritable0(int minWritableBytes) {
        ensureAccessible();
        // todo   writableBytes 是 capacity - writeIndex
        if (minWritableBytes <= writableBytes()) {
            return;
        }
        // todo 如果要写的 比 最大值 - 已经使用的还大, 直接抛出异常
    ...
    public byte readByte() {
        ...
        int i = readerIndex;
        // todo 这种以下划线开头的方法，都是留给子类去实现的抽象 方法
        byte b = _getByte(i);
        readerIndex = i + 1;
        return b;
    }
    ...
    // todo 链式调用风格， 每次写完后，返回自己。方便下一次的调用
    @Override
    public ByteBuf writeByte(int value) {
        ensureWritable0(1);   // todo  检查底层的数组是否还有一个可用空间 , 进去查看
        // todo 同样虾下面的 _开头的方法，是抽象方法
        _setByte(writerIndex++, value);  //todo 把值写到特定的位置上
        return this;
    }
