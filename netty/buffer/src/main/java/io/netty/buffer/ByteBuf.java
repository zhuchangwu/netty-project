 ...
 * todo 是可以对零个或多个字节的序列 , 进行顺序的访问
 * This interface provides an abstract view for one or more primitive byte
 * arrays ({@code byte[]}) and {@linkplain ByteBuffer NIO buffers}.
 * todo 这个接口对原始的 byte[] 或者 NIO的 ByteBuffer 提供了一个抽象的 访问视图
 *
 ...
 * constructor.
 * todo 建议使用 Unpooled 去创建 ByteBuf , 而不是使用那些具体实现类的 自带的构造方法
 *
 * <h3>Random Access Indexing</h3>  todo 随机访问索引
 *
 ...
 *  todo 和原生的byte数组一样,它的第一个索引是 0 , 最后一个索引是 capacity()-1
 *  todo 你可以像下面这样,去 迭代一个数组中的 byte信息, 而不用管他是怎么实现的
 * <pre>
 * {@link ByteBuf} buffer = ...;
 ...
 * </pre>
 *
 * <h3>Sequential Access Indexing</h3> todo 顺序访问索引
 *
 ...
 * todo byte 提供了两个指针变量来 提供顺序的读和写的操作, 一个是用于读操作的 readerIndex 和一个writeIndex 用于写操作的
 * todo 这个NIO中的ByteBuffer 是明显不同的, 因为NIO中的ByteBuffer只有一个变量  position,去定位是读还是写, 而且,在进行读写操作的转换时,
 *  todo                                还的调用    flit() 将limit和position之间进行重新的设定
 * <pre>
 *      todo 下面这个图 就展示了  一个byteBuf 是如何被两个指针分成三个区域的
 *      +-------------------+------------------+------------------+
 *      | discardable bytes |  readable bytes  |  writable bytes  |
 * todo |   可丢弃的(读过)  |   即将被读取的    |  可写的字节       |
 *      +-------------------+------------------+------------------+
 *      |                   |                  |                  |
 ...
 * <p>todo 如果 read的参数是 ByteBuf, 并且没有指定任何的 index, 意思就是 把read出来的数据 写到了参数位置上的 ByteBuf
 *
 * If there's not enough content left, {@link IndexOutOfBoundsException} is
 * raised.  The default value of newly allocated, wrapped or copied buffer's
 * {@link #readerIndex() readerIndex} is {@code 0}.
 * todo 如果空间不够用了 就抛出 IndexOutOfBoundsException
 * todo 新创建的 ByteBuf readerIndex 是 0
 * <pre>
 * // Iterates the readable bytes of a buffer.
 * {@link ByteBuf} buffer = ...;
 * while (buffer.isReadable()) {
 *     System.out.println(buffer.readByte());
 * } todo 这个例子,展示了如何迭代一个可读字节数组的循环  , 底层就是在判断, readindex   >  writeindex
 * </pre>
 *
 * <h4>Writable bytes</h4> todo 可写的字节
 *
 * This segment is a undefined space which needs to be filled.
 *  todo 这个块是一个未定义的空间,他需要被填充
 *
 * Any operation
 * whose name starts with {@code write} will write the data at the current
 * {@link #writerIndex() writerIndex} and increase it by the number of written
 * bytes.
 * todo 以write开头的任何操作都是将数据写入到,当前数组的 writerIndex位置, 然后增加它的下标
 *
 * If the argument of the write operation is also a {@link ByteBuf},
 * and no source index is specified, the specified buffer's
 * {@link #readerIndex() readerIndex} is increased together.
 *  todo 如果 write的参数位置上是一个 ByteBuf, 并且也没有index的指定, 那么实际上就是往 ByteBuf里面写, 写入的位置从 readerIndex开始
 * <p>
 ...
 * todo 依然, 如果写入时没有足够的容量就是抛出 IndexOutOfBoundsException 索引越界异常
 * todo 对于一个新创建的 byteBuf writerIndex 位置为 0
 * todo 如果这个 byteBuf是拷贝的, writerIndex = capacity
 * <pre>
 * // Fills the writable bytes of a buffer with random integers. todo 下面的例子是使用一个随机的整数填充一个writeableBytes
 * {@link ByteBuf} buffer = ...;
 * while (buffer.maxWritableBytes() >= 4) { todo 为什么要>=4   因为在java中一个整数 占据四个字节
 *     buffer.writeInt(random.nextInt());
 * }
 * </pre>
 *
 * <h4>Discardable bytes</h4> todo 可丢弃的字节
 *
 * This segment contains the bytes which were read already by a read operation.
 * todo 这个区域包含是已经被读过的字节
 * Initially, the size of this segment is {@code 0}, but its size increases up
 * to the {@link #writerIndex() writerIndex} as read operations are executed.
 *  todo 一开始 这个区域的大小为 0, 但是随着 read的操作, 它的大小会增加到 writerIndex
 * The read bytes can be discarded by calling {@link #discardReadBytes()} to
 * reclaim unused area as depicted by the following diagram:
 * todo  可用通过使用`discardReadBytes()  使得读过的区域的字节所占的位置让出来
 * <pre>
 *
 *  BEFORE discardReadBytes()
 *  todo 这是调用 `discardReadBytes()`之前的状态
 *      +-------------------+------------------+------------------+
 *      | discardable bytes |  readable bytes  |  writable bytes  |
 *      +-------------------+------------------+------------------+
 *      |                   |                  |                  |
 ...
 * todo 可以看到,调用完成之后, 原来的 discardable的区域已经没有了 , 可写的空间获得扩容
 *      +------------------+--------------------------------------+
 ...
 * todo 请注意, 在调用完了 `discardReadBytes()`  方法后. 对于可写入的字节的内容并没有任何的保证
 * todo  在大多数的情况下, 可写入的字节的是不会被移动的, 甚是可能会被填充上不同的数据,这取决于底层 buffer 的实现
 * todo  啥意思呢? 就是说,当我们调用discardReadBytes()方法后, 原来的中间部分的readable bytes,赋值一份存放到 开头的discardable bytes
 * todo  然后, 把中间位置的 readable bytes 转换成  writable bytes , 原来的数据依然可能保留着, 但是可以被新的数据替换掉


 * <h4>Clearing the buffer indexes</h4> todo 清除buffer的索引
 *
 ...
 * todo 你可以通过调用clear()方法 ,  将 readerIndex和 writerIndex 都设置成0
 * todo 它仅仅是改变了两个指针的位置,而没有清空 buffer中的内容
 * todo  注意, 它和JDK底层的 butebuffer的语义是不一样的, 下面是结构图
 *
 * <pre>
 *  BEFORE clear()
 * todo 调用clear()之前
 ...
 *      +---------------------------------------------------------+
 *      |                                                         |
 *todo    0 = readerIndex = writerIndex            <=          capacity
 * </pre>
 *
 * <h3>Search operations</h3> todo 搜索操作
 *
 ...
 * implementation.
 * todo 可以使用indexOf(int, int, byte)     #bytesBefore(int, int, byte)  bytesBefore(byte) 等进行搜索
 *
 ...
 * {@code readlimit}.
 * todo 在每个 buffer中都存在着两个标志位, 因为Bytebuf中有两个指针, 分别存放着  readerIndex和writerIndex的位置
 * todo 你可以使用 reset来初始化他们中任意一个的位置
 *
 * <h3>Derived buffers</h3> todo 衍生的buffer
 *
 * You can create a view of an existing buffer by calling one of the following methods:
 * todo 你可以通过调用如下的方法,创建一个已有的buffer的视图
 * <ul>
 *   <li>{@link #duplicate()}</li> todo 复制
 *   <li>{@link #slice()}</li>
 ...
 * <p> todo 新衍生出来的 buffer 和原来的buffer共享一份数据,(浅复制) 但是它的 readerIndex  writerIndex  marks 是它自己的
 *
 * In case a completely fresh copy of an existing buffer is required, please
 * call {@link #copy()} method instead.
 * todo 如果你想进行深度的赋值, 建议使用` copy()`
 *
 ...
 *
 * <h3>Conversion to existing JDK types</h3> todo 转换成 jdk的类型
 *
 * <h4>Byte array</h4> todo 字节数组
 *
 * If a {@link ByteBuf} is backed by a byte array (i.e. {@code byte[]}),
 * you can access it directly via the {@link #array()} method.
 *  todo 如果一个ByteBuf  是由一个byte[]来维护的话, 你就可以使用 array() 方法,获取底层的字节数组
 *
 * To determine if a buffer is backed by a byte array, {@link #hasArray()} should be used.
 *  todo 通过  hasArray() 方法,来确定是否真的存在那个数组
 *
 * <h4>NIO Buffers</h4> todo NIO的Buffer
 *
 ...
 * todo  同样如果一个 ByteBuf 底层是由一个 NIO的ByteBuffer 实现的, 就可以通过 nioBuffer() 来获取这个数组
 * todo 同样适用 nioBufferCount() 判断是否存在这个数组
 *  todo 补充, 我们知道,对应NIO的ByteBuffer来说,本身有两种数据类型, 1,堆内 2,堆外
 *  todo 对于netty来说, 其实他有三种类型 1. 堆上(被jvm掌控)  2. 堆外(直接缓冲区, 在操作系统实现的) 3. 复合缓存区 (可以包含任意多个 堆内缓从和堆外缓冲)
 *
 *
 * <h4>Strings</h4>  todo 如何将Bytebuf转换成字符串
 *
 * Various {@link #toString(Charset)} methods convert a {@link ByteBuf}
 * into a {@link String}.  Please note that {@link #toString()} is not a
 * conversion method.
 *   todo netty 提供了 各种各样的 tostring的方法 , 可以将ByteBuf 转换成字符串
 *   todo 请注意, 只有 toString(字符集) 才是真正的转换的方法, 单独的不带参数的 toString() 不是转换的方法
 * <h4>I/O Streams</h4>
 *
...
// todo netty的重要数据容器类  和jdk原生的byteffer很像, 都是用来存储项目中的数据
// todo 这个抽象类大部分的方法,都是抽象的

// todo 他有一个抽象子类, 是我们日常开发时使用比较多的 AbstractByteBuf 它本身就是一个抽象类, 但是它实现了大量的方法
public abstract class ByteBuf implements ReferenceCounted, Comparable<ByteBuf> {
     ...
     */
    public abstract int readerIndex();   // todo 获取当前buffer 的  readerIndex 值

    /**
     * Sets the {@code readerIndex} of this buffer.
     * todo 直接设置 当前 buffer 的 readerIndex -- 设置值, 相当于是 setReaderIndex(int readerIndex)
     ...
     */  // todo 返回 当前 buffer的 writerIndex
    public abstract int writerIndex();

    /**
     * Sets the {@code writerIndex} of this buffer.
     * todo 设置当前buffer的  writerIndex 带参数的相当于是在设置值
     * @throws IndexOutOfBoundsException
     ...
     *         if the specified channel threw an exception during I/O
     */
    // todo 默认情况 进入 PooledDirectByteBuf
     ...
     * {@link #arrayOffset()}.
     *  todo 如果这个方法返回 true , 说明当前的ByteBuf底层是由字节数组实现的
     *  todo  可以安全的使用  array()   和   arrayOffset()
     */
    public abstract boolean hasArray();

    /**
     * Returns the backing byte array of this buffer.
     * todo 返回 ByteBuf 底层的 字节数组
     * @throws UnsupportedOperationException
