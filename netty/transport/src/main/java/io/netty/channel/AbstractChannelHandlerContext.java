...
// todo  这个ChannelHandlerContext是实现类, 实现了大部分的功能
// /todo 继承了  DefaultAttributeMap  !!! 因为这个类维护着  他和 Channel的 Attribute内容
abstract class AbstractChannelHandlerContext extends DefaultAttributeMap
    ...
    static void invokeChannelActive(final AbstractChannelHandlerContext next) {
        ...
        if (executor.inEventLoop()) {
            ...
            // todo 获取当前的handler上下, 进而获取它对应的  执行器, 执行chanel激活的方法
            executor.execute(new Runnable() {
                @Override
                public void run() {
                    next.invokeChannelActive();
    ...
    // todo 调用 当前上下文中的 handler 的 write方法, 新型事件传播-- 最终会传播到我们字节的解码器 --> header写出
    ...
    // todo 传播flush事件, 一般我们都不会重写这个 方法, 然后他就一直往前传, 一直到 head节点
    ...
    // todo 下面的attr实际上是用的 channel的 attr
    @Override
