 ...
 * <p> todo ChannelOutboundBuffer 数据结构是用来存储 添加出站请求 的数据
 *
 * All methods must be called by a transport implementation from an I/O thread, except the following ones:
 * todo 除以下方法外，所有方法都必须由I/O线程的传输实现调用：
 * <ul>
...
public final class ChannelOutboundBuffer {
    ...
    private Entry flushedEntry;  // todo 链表中已经被flush过的entity

    // The Entry which is the first unflushed in the linked-list structure
    private Entry unflushedEntry; // todo 第一个没有被flush过的实体

    // The Entry which represents the tail of the buffer
    private Entry tailEntry; // todo 尾指针
    ...
    // todo 通过自旋锁机制, 传播 ChannelWritabilityChanged(invokeLater);  传播channel不能写状态
