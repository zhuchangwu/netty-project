...
/**
 *  todo  是NioServerSocketChannel的第二代父类, AbstractNioChannel
 *
 *  // todo ？？？？ 为啥这么说？
 *  todo  本类是 使用基于选择器Selector 的方法的 实现的抽象基类。换言之, 它相当于对NIO中    Selector    的又一层封装
...
// todo SelectableChannel原生的jdk的channel
    ...
    protected SelectableChannel javaChannel() {
        // todo  这里的ch 就是上面我们传递进去的  ServerSocketChannel
        // todo  创建 客户端的channel ch是
        return ch;
    }
    ...
    // todo doRegister()的真正逻辑
    @Override
    protected void doRegister() throws Exception {
        boolean selected = false;
        for (;;) {
            try {
                // todo  找到了!!! 调用了jdk底层的 注册逻辑
