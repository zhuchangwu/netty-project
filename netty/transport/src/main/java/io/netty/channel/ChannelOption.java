 ...
 * way.
 * todo 让我们以一种 类型安全的模式 配置ChannelConfig
 *
 * Which {@link ChannelOption} is supported depends on the actual implementation
 * of {@link ChannelConfig} and may depend on the nature of the transport it belongs
 * to.
 * todo 具体是哪一种 ChannelOption 取决于 实际实现的是哪一种 ChannelConfig, 也取决于它依赖的传输层
 *
 * @param <T>   the type of the value which is valid for the {@link ChannelOption}
 */

// todo 主要是使用它进行TCP底层的设置项, 去完成设定
 // todo <T> 表示设置项对应的值的类型 , 进入它的父类

  // todo ChannelOption 本身只维护key , 而值并不是他自己维护的! 它的Key 可以起到校验的 作用, 比如, T == Boolean, 那么获取出来的ChannelOnption的常量类型为布尔类型
public class ChannelOption<T> extends AbstractConstant<ChannelOption<T>> {

    // todo 下面这段代码是 在ChannelOption 中创建出  常量池ConstantPool, 在这个常量池中维护这具体的 ChannelOption的信息
    ...
    // todo  下面是Netty为我们设置的关于TCP 常量的名字
