 ...
 *  todo  他一个按照分隔符进行解码的 解码器, 尤其是按照null, 空, 换行
 ...
 * the following data in the buffer:  todo 它允许你定义多个分隔符, 如果他在解码时, 发现了多个分隔符, 他就会默认选择最细力度的 帧
 * <pre>
 * +--------------+
 * | ABC\nDEF\r\n |
 * +--------------+
 ...
 *  todo Delimiters.lineDelimiter() 就会根据 换行进行分隔 也就是 \n , 结果如下
 * <pre>
 * +-----+-----+
 * | ABC | DEF |
 * +-----+-----+
...
public class DelimiterBasedFrameDecoder extends ByteToMessageDecoder {
    ...
    protected Object decode(ChannelHandlerContext ctx, ByteBuf buffer) throws Exception {
        ...
        for (ByteBuf delim: delimiters) {
            // todo 遍历所有的分隔符, 找到最小分隔符 , 假如说, 存在两个分隔符A B, 从当前的readIndex开始到A有2个字节,到B有3个字节,就会按照A分隔
        ...
        if (minDelim != null) {
            ...
            // todo 当前找到的数据包 > 提前定义的长度
            if (minFrameLength > maxFrameLength) {
                // Discard read frame.
                // todo 丢弃
                buffer.skipBytes(minFrameLength + minDelimLength);
                fail(minFrameLength); // todo 抛出异常
                return null;
            }
